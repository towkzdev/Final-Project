...
const TabLDW = () => {
    ...
    const minVal = Math.min(...scoreyear.map(item => item.ldw_dscore))
    //console.log"ค่าต่ำสุด", minVal)

    const maxVal = Math.max(...scoreyear.map(item => item.ldw_dscore));
    ...
    const minVal5 = Math.min(...scoreyear.map(item => item.ldw_tscore))
    //console.log"ค่าต่ำสุด", minVal)
    const maxVal5 = Math.max(...scoreyear.map(item => item.ldw_tscore));
    //console.log"ค่าสูงสุด", maxVal)

    const data = {
        labels: ["ด้านการเขียน", "ค่าเฉลี่ย", "ค่าสูงสุด", "ค่าต่ำสุด"],
        datasets: [
            {
                ...
                data: [scorestd.ldw_dscore, avg, maxVal, minVal]
            },

            {
                ...
                data: [scorestd.ldw_tscore, avg5, maxVal5, minVal5]
            },
    ...
    const minVal4 = Math.min(...scoreyear.map(item => item.ldw_group))

    const maxVal4 = Math.max(...scoreyear.map(item => item.ldw_group));



    const data3 = {
        labels: ["ด้าน การเขียน", "ค่าเฉลี่ย", "ค่าสูงสุด", "ค่าต่ำสุด"],
        datasets: [

            {
                ...
                data: [scorestd.ldw_group, avg4, maxVal4, minVal4]
            },
        ],
    };
        ...
        <div className="alltab-bg">
            {adhddata === null ? <div style={{ textAlign: "center" }}>ไม่พบข้อมูลการประเมินนักเรียน</div> : (
                <div>
                    ...
                    <h5 style={{ textAlign: "center" }}>พฤติกรรมภาวะบกพร่องทางการเรียนรู้ด้านการเขียน (KUS-SI Rating Scale 3: LD-Written Expression)  </h5> <br />
                        ...
                        <div>
                            <div style={{ display: "flex" }}>
                                <Row style={{ backgroundColor: "whitesmoke", padding: "20px 35px 40px 35px", borderRadius: "20px", boxShadow: "1px 2px 2px 0px rgb(158, 158, 158)", width: "50%", marginRight: "30px" }}>
                                    ...
                                    <h6 style={{ textAlign: "center" }}>ด้านการเขียน</h6> <br />
                                    <Line data={data} />
                                ...
                                <Row style={{ backgroundColor: "whitesmoke", padding: "40px 35px 40px 35px", borderRadius: "20px", boxShadow: "1px 2px 2px 0px rgb(158, 158, 158)", width: "50%" }}>
                                    <h5 style={{ textAlign: "center" }}>ปีการศึกษา {schoolyear2} / {term2}</h5> <br />
                                    <h6 style={{ textAlign: "center" }}>กลุ่ม ด้านการเขียน</h6> <br />
                                    <Bar data={data3} />
                                </Row>
                            </div>
