...
const School = () => {
    ...
    useEffect(() => {
        axios
            .get(`${process.env.REACT_APP_API}/studentcontact/${schoolid}`,
                {
                    headers: {
                        authtoken,
                    }
    ...
    */
    ...
    const data = {
        labels: ["ค่า Avg (โรงเรียน)", "ค่าMax (โรงเรียน)", "ค่าMin (โรงเรียน)"],
        datasets: [
            {
    ...
    const data3 = {
        labels: ["ค่า Avg (โรงเรียน)", "กลุ่ม1(คน)", "กลุ่ม2(คน)", "กลุ่ม3(คน)", "กลุ่ม4(คน)"],
        datasets: [
            {
    ...
    return (
        <div>
            ...
            {scoreyear.length === 0 ? <div className="alltab-bg" style={{ textAlign: "center" }}>-ค้นหาปีการศึกษา-</div> : (

                <div className="alltab-bg">
                    <h5 style={{ textAlign: "center" }}>ปีการศึกษา {schoolyearvalue.map(item => item.schoolyear)} เทอม {termvalue.map(item => item.term)} </h5>
                    <br />
                    <div className='d-flex justify-content-center'>
                        <a style={{ color: "red" }}>จำนวนนักเรียนที่ประเมินทั้งหมด    {scoreyear.length}/{allstudent.length} คน</a>
                    </div>
                    <br />
